...
func TestSniper_SyncOrders(t *testing.T) {
	...
	s := NewSniper(detail, &MockStrategy{}, policy, ord.EXCHANGE_TOSHO, nil)

	// 1. 注文を発注した直後の状態（PENDING ID）
	pendingID := ord.GenerateLocalID()
	internalOrder := ord.NewOrderPtr(pendingID, "9434", ord.ACTION_BUY, 2000, 100)
	...
	s.SyncOrders(ord.Orders{Orders: extOrders})

	// 4. キャンセル送信中の状態にする
	internalOrder.Status = ord.ORDER_STATUS_CANCEL_SENT
	...
	if internalOrder.Status != ord.ORDER_STATUS_CANCELED {
...
func TestSniper_SyncOrders_Executions(t *testing.T) {
	...
	s := NewSniper(detail, &MockStrategy{}, policy, ord.EXCHANGE_TOSHO, nil)
	realID := "order-456"
	internalOrder := ord.NewOrderPtr(realID, "9434", ord.ACTION_BUY, 2000, 100)
	s.Orders = append(s.Orders, internalOrder)
